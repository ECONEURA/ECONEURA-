...
for (const f of SRC()) {
  ...
  // Purga referencias de claves y variables en cliente
  s = s
    .replace(/\bGW_KEY\s*:\s*readVar\([^)]*\),?\s*\n/g, () => {
      stats.env_purged++;
      return '';
    })
    ...
    .replace(/process\.env\.(NEURA_GW_KEY|LA_SHARED_KEY)\b/g, () => {
      stats.env_purged++;
      return 'undefined';
    })
    .replace(/import\.meta\.env\.(VITE_NEURA_GW_KEY|VITE_LA_SHARED_KEY)\b/g, () => {
      stats.env_purged++;
      return 'undefined';
    });

  // Authorization: usar bearer global o SIMULATED
  s = s.replace(/Authorization["']?\s*:\s*`Bearer\s*\$\{[^}]*GW_KEY[^}]*\}`/g, () => {
    stats.auth_rewrites++;
    return 'Authorization: `Bearer ${(globalThis as any).__ECONEURA_BEARER || "SIMULATED"}`';
  });

  // Telemetría: reemplazar implementación por NOOP en cliente
  s = s.replace(/async\s+function\s+logActivity\([^)]*\)\s*\{[\s\S]*?\n\}/m, () => {
    stats.log_noop++;
    return 'async function logActivity(){ /* NOOP client; server-side only */ }';
  });
...
// Post-scan: detectar posibles fugas en UI
const files = SRC();
...
fs.writeFileSync(
  'docs/audit/CLIENT_SECRET_PURGE.md',
  `# CLIENT_SECRET_PURGE
