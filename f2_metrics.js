...
for (const f of Object.values(cov)) {
  if (!f?.lines) continue;
  tl.total += f.lines.total || 0;
  tl.covered += f.lines.covered || 0;
}
...
const byFile = {};
(vit.testResults || []).forEach(tr => {
  if (tr.status !== 'failed') return;
  const k = tr.testFilePath || 'unknown';
  byFile[k] = (byFile[k] || 0) + 1;
});
const top = Object.entries(byFile)
  .sort((a, b) => b[1] - a[1])
  .slice(0, 10)
  .map(([f, c]) => ({ file: f, fails: c }));
