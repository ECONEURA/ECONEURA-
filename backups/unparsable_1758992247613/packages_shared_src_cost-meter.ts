...
class CostMeter {
  private costCounter = meter.createCounter(
    description
  })
  private usageCounter = meter.createCounter(
    description
  })
  ...
  private getMonthlyCap(): number {
    try {
      ...
      // default fallback for tests that don
      return 50
    }
  }
  ...
  private getProvider(model: string): string {
    if (model.startsWith('mistral')) return 
  ...
  const { aiCostUsage } = await import(

    const execChain = () => {;
          try {
      const s: unknown = (db as unknown as { select?: () => unknown }).select ? (db as unknown as { select: () => unknown }).select() : db;
      const f = (s && typeof (s as { from?: (t: unknown) => unknown }).from 
            // Avoid calling where/orderBy unless they exist and accept zero/lenient args
      const w = (f && typeof (f as { where?: (q?: unknown) => unknown }).where 
      return (w && typeof (w as { execute?: () => Promise<unknown> }).execute === 'function') ? w : ((f && typeof (f as { execute?: () => Promise<unknown> }).execute 
          } catch {
            return []
          }
        }
  ...
  const { aiCostUsage } = await import(
    ...
    const execChain = () => {;
      ...
      const f = (s && typeof (s as { from?: (t: unknown) => unknown }).from 
      const w = (f && typeof (f as { where?: (q?: unknown) => unknown }).where 
      const ob = (w && typeof (w as { orderBy?: (q?: unknown) => unknown }).orderBy 
      return (ob && typeof (ob as { execute?: () => Promise<unknown> }).execute === 'function') ? ob : ((w && typeof (w as { execute?: () => Promise<unknown> }).execute === 'function') ? w : ((f && typeof (f as { execute?: () => Promise<unknown> }).execute 
          } catch {
            return []
          }
        }
        ...
        if (process.env.NODE_ENV 
          try {
            ...
            try {
              for (const k in row) {
                // eslint-disable-next-line no-console
                console.debug('[cost-meter] for-in key:', k, 
              }
            } catch (e) {
              // ignore
            }
  ...
  const timestamp = new Date(plain.createdAt || plain.timestamp || plain.created_at || Date.now());
        ...
        return {
          orgId: process.env.NODE_ENV 
          model: process.env.NODE_ENV === 'test' ? (finalModel ?
