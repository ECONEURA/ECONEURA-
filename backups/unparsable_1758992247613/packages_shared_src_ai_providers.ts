...
export class LLMProviderManager {;
  ...
  private initializeDefaultProviders(): void {
    ...
    this.addProvider({
      ...
      healthEndpoint: undefined, // OpenAI doesn
      models: [
        {
          id
          name
  ...
  getBestProvider(requirements: {
    ...
    return candidates.sort((a, b) => {
      ...
      if (requirements.preferEdge) {
        if (a.type === 'edge' && b.type 
        if (b.type === 'edge' && a.type 
      }
      ...
      if (healthA?.status === 'healthy' && healthB?.status 
      if (healthB?.status === 'healthy' && healthA?.status 
  ...
  private async checkProviderHealth(provider: LLMProvider): Promise<void> {
    ...
    try {
      ...
      if (provider.healthEndpoint && provider.type 
