...
(globalThis as any).__TEST_CREATE_ROOT = (el: any) => ({ render: () => { const n = document.createElement('div'); n.setAttribute('data-client-rendered', '1'); el.appendChild(n) } })
...
test('main client mount works when forced', async () => {
  ...
  const resolveMount = (m: any) => {
    ...
    if (m.default) {
      ...
      if (typeof m.default === 'function' && typeof (m.default as any).mountApp === 'function') return (m.default as any).mountApp
      if (m.default.default && typeof m.default.default.mountApp === 'function') return m.default.default.mountApp
    }
    try {
