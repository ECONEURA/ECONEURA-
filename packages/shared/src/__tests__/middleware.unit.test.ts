...
describe('middleware', () => {
  ...
  test('requestLogger logs on finish', () => {
    ...
    const res: any = { on: (ev: string, fn: any) => { if (ev === 'finish') fn() }, statusCode: 200 }
    const next = vi.fn()
  ...
  test('errorHandler decorates writeHead', () => {
    const req: any = {}
    const res: any = { headersSent: false, setHeader: vi.fn(), writeHead: function(s: number) { this._status = s } }
    const next = vi.fn()
