...
for (const f of files) {
  ...
  s = s
    .replace(/^\s*GW_KEY:\s*readVar\([^)]*\),?\s*$/m, () => {
      S.env_refs_removed++;
      return '';
    })
    .replace(/^\s*LA_KEY:\s*readVar\([^)]*\),?\s*$/m, () => {
      S.env_refs_removed++;
      return '';
    })
    .replace(
      /\b(import\.meta\.env\.(VITE_NEURA_GW_KEY|VITE_LA_SHARED_KEY)|process\.env\.(NEURA_GW_KEY|LA_SHARED_KEY))\b/g,
      () => {
        S.env_refs_removed++;
        return 'undefined';
      }
  ...
  s = s
    .replace(
      /if\s*\(\s*!env\.GW_URL\s*\|\|\s*!env\.GW_KEY\s*\)/,
      'if (!env.GW_URL || !(globalThis as any).__ECONEURA_BEARER)'
    )
    .replace(/['"]Authorization['"]\s*:\s*`Bearer\s*\$\{env\.GW_KEY\}`/g, () => {
      S.auth_fixed++;
  ...
  s = s.replace(/async\s+function\s+logActivity\([^)]*\)\s*\{[\s\S]*?\n\}/m, () => {
    S.log_noop++;
    return 'async function logActivity(){ return; }';
  });
